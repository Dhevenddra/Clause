...
## DEC-013 — Post-freeze ops hardening + judge test library (content, guardrails — not features)
**Date:** 2026-07-11 · **Status:** Locked
Post-H6 additions justified as operational necessity + judge-facing content, not scope creep: (a) `/adjudicate` rate limiting — 15/IP/hr, 80 global/hr, concurrency 2 — because the public URL fronts a $10/hr metered GPU and the founder correctly flagged spam risk; worst-case burn self-caps at ~$10–14/hr; (b) patient retry (240s budget, 15s interval) through Fireworks `DEPLOYMENT_SCALING_UP` 503 — cold-start previously returned an instant error to the first user; verified live through a 139s GPU wake; (c) `demo/library/` motor + travel policy/claim pairs, verified on real Gemma (FLAG 6/6 cited; DENY 1/1 incl. the 3,400m-vs-3,000m clause) so judges have varied, copy-pastable material; (d) Astryx + Litmus-taste redesign explicitly deferred to `docs/v0.2-DESIGN-NOTES.md` — DEC-005/008/009 stand for v0.1. Repo PII-scanned: clean; stray reference screenshot scrubbed from history minutes after an accidental commit.

## DEC-014 — Pitch video produced by automation, not on-camera recording
**Date:** 2026-07-11 · **Status:** Locked
Founder chose no on-cam footage. Pipeline (all free/local, committed in `video/`): edge-tts narration (`en-US-AndrewNeural`, 8 segments, 117.7s), Playwright-scripted Chromium recording of the LIVE deployed URL with timestamped scene marks (deny flow → verdict → citation highlight → adversarial claim → audit record), ink-on-paper title cards rendered from HTML, ffmpeg per-scene cuts (narration-length-matched, freeze-extended) + concat. Output: `video/CLAUSE-pitch.mp4` — 2:02, 1280×720 H.264/AAC, 9.4 MB. HeyGen/avatar tools rejected: paid, slower, and judges need the product working, not a face. Every demo shown in the video is real Gemma inference on the judge-facing URL.
