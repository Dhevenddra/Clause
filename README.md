...
**Live demo:** https://clause-4vv4.onrender.com (free-tier host — first hit after idle takes ~1 min while the container and the GPU deployment wake)
...
## Quick start
```bash
cp .env.example .env        # add your FIREWORKS_API_KEY and MODEL_ID (see below)
...
**MODEL_ID:** Fireworks serves Gemma 4 on dedicated on-demand deployments (it is no longer on the serverless tier). Create one in the Fireworks dashboard (Gemma 4 31B IT NVFP4 fits a single GPU; autoscale min 0 protects your credits) and reference it as
`accounts/fireworks/models/gemma-4-31b-it-nvfp4#accounts/<your-account>/deployments/<id>`.
Any OpenAI-compatible endpoint serving a Gemma model works the same way — see the MI300X section below.
